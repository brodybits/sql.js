...
exports.test = function(sql, assert){
    ...
    db.run("CREATE TABLE data (nbr, str, no_thing); INSERT INTO data VALUES (5, '粵語😄', NULL);");
    stmt = db.prepare("SELECT * FROM data");
    stmt.step(); // Run the statement
    assert.deepEqual(stmt.getColumnNames(), ['nbr','str','no_thing'], 'Statement.GetColumnNames()');
    var res = stmt.getAsObject();
    assert.strictEqual(res.nbr, 5, 'Read number');
    assert.strictEqual(res.str, '粵語😄', "Read string");
    assert.strictEqual(res.no_thing, null, "Read null");
    assert.deepEqual(res, {nbr:5, str:'粵語😄', no_thing:null}, "Statement.getAsObject()");
    stmt.free();
